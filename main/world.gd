...
func _ready():
	...
	spawn_interactable("bronze_dagger", "random", "");
...
func _on_mined_item(mining_target):
	...
	if mining_target != null:
		...
		object.queue_free();
		spawn_interactable(object.name, "environment", object.position)
...
func spawn_interactable(item_to_spawn, location, position):
